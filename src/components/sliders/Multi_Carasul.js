  ...
  return (
    ...
    >
      {offercard.map((item) => (
        ...
        >
          <Image src={item.img} alt={item.title} width={367} height={245} className="w-[347px] h-[245px] object-cover rounded-[23px] mx-auto" />
          <div className="sm:text-start text-center">
            <h5 className="md:text-[26px] text-[20px] font-medium font-Helvetica pb-4">{item.title}</h5>
            <p className="md:text-[17px] text-base font-medium pb-4 font-Helvetica text-[#2c3e50]">{item.para}</p>
            <h6 className="pb-7 md:text-[17px] text-base font-medium font-Helvetica"><b>{item.price}</b></h6>
